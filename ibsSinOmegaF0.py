...
sValsAll = np.linspace(0,1,M,endpoint=False)

def beta_s(s):
    return np.sin(np.pi/2*s)

betaValsAll= beta_s(sValsAll)*2*np.pi
...
plt.title("$\omega_{F}=0$, $T=$"+str(T)+", pumping = "+str(dis))
plt.xlabel("$t/T$")
plt.savefig("ibssinaomegaF0"+"c1"+str(c1)+"c2"+str(c2)+"betaNum"+str(M)+"blochNum"+str(N)+"displacement.png")
#plt.close()

pumpings_np = np.array(pumpings)
# np.save("pumping.npy",pumpings_np)
