...
#this script computes pumping within 1 period
...
for q in range(0,Q):
    ...
    tmp1 = np.exp(-1j * (V * dt * np.cos(2 * np.pi * alpha * locations - betaVal) * np.cos(
        Omega * tq) + omegaF * locations * dt)) * state
    tmp2 = np.fft.ifft(tmp1, norm="ortho")
    tmp3 = np.exp(-1j * J * dt * np.cos(kTotal)) * tmp2
    state = np.fft.fft(tmp3, norm="ortho")
    psiOnePeriod.append(state)

tPumpEnd=datetime.now()
print("evolution time: ",tPumpEnd-tPumpStart)
outDir="./thesis/oneperioddataFrameT1"+str(T1)+"a"+str(a)+"b"+str(b)+"/"
Path(outDir).mkdir(parents=True,exist_ok=True)

outPsidata=np.array(psiOnePeriod)
pdPsi=pd.DataFrame(data=outPsidata)
pdPsi.to_csv(outDir+"a"+str(a)+"b"+str(b)+"band"+str(bandNum)+"psiOnePeriod.csv",index=False)